...
class PassportController(passportService: PassportService, peopleService: PeopleService)(implicit val tokenVerifier: TokenVerifier ) {
  ...
  val `GET /passport/me`: Endpoint[Json] = get(passport :: me :: tokenHeader) { (token: String) =>
    auth(token) { user =>
  ...
  val `GET /passport/employeeId`: Endpoint[Json] = get(passport :: string :: tokenHeader) { (employeeId: String, token: String) =>
    auth(token) { user =>
